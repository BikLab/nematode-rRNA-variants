...
### Subset OTUs/ASVs and change to Wide Format ### 
...
### Make a new column with all the OTUs/ASVs labels ### 
...
### Plot using GGPLOT ### 
PLOT_Chromadoridae <- ggplot(Label_DF) + geom_segment(aes(x="1", xend="2", y=RelAbundance.x, yend=RelAbundance.y, col=factor(Species.x)), size=0.75, show.legend = T) +
  ...
  ylim(1,100) + ggtitle("Chromadoridae ASVs/OTUs")
PLOT_Chromadoridae
