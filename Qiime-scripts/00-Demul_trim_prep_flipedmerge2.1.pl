...
while (<INBC>) {
	...
	if ( length $mapping_file{ $line[0] }{"BarcodeSequence"} != 15 ) {
		print STDERR "*** Warning *** Barcode sequence $mapping_file{ $line[0] }{\"BarcodeSequence\"} is not 15 bases long\n";
	}
	if ( length $mapping_file{ $line[0] }{"ReverseBarcode"} != 15 ) {
		print STDERR "*** Warning *** Barcode sequence $mapping_file{ $line[0] }{\"ReverseBarcode\"} is not 15 bases long\n";
	}
